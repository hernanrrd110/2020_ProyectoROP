...
% rgb = imread('prueba.png');
% rgb = imread('IMG_0349_proc.png');
% rgb = imread('IMG_1480_proc.png');
% rgb = imread('IMG_6147_proc.png');

% === Imagen en RGB y en escala de grises
rgb = imread('IMG_1256.PNG');
...
im_filt_marcada = zeros(round(tamanio(1)*1.20),round(tamanio(2)*1.20));
...
im_filt_marcada(floor(tamanio(1)*0.10):floor(tamanio(1)*1.1)-1,...
...
imshow(im_filt_marcada);

% Intervalo de radio del circulo a detectar
radio1 = 440;
radio2 = 450;
[centers, radii] = imfindcircles(im_filt_marcada,[radio1 radio2],...
...
for i = 1:size(rgb_enmarcada,1)
    for j = 1:size(rgb_enmarcada,2)
        ...
        if((i-centers(1))^2+(j-centers(2))^2 > radii^2)
            im_cortada(i,j,:) = 0;
        end
    end
end
